...
constexpr int BR_LEFT      = 408; // Координата X для иконок битрейта   
constexpr int BR_TOP       = 125; // 142 Координата Y для иконок битрейта
constexpr int VU_RECT_X_LEFT   = 10;// Координата X для левого VU-метра
constexpr int VU_RECT_X_RIGHT  = VU_RECT_X_LEFT ;// Координата X для правого VU-метра
constexpr int VU_RECT_Y        = 125;// Координата Y для VU-метров
constexpr int VU_RECT_WIDTH    = 29;// Ширина прямоугольника VU-метра
constexpr int VU_RECT_HEIGHT   = 13;// Высота прямоугольника VU-метра
...
void FillWidget::_draw(){
  ...
  if (display.mode() != STATIONS) {
    dsp.drawRGBBitmap(346, 00, wifi, 46, 32);
    dsp.drawRGBBitmap(250, 0, volume, 38, 32);

    dsp.drawRGBBitmap(0,0, ip, 36, 36);


    
  }
 dsp.drawFastHLine(0, 37, 480,  config.theme.div);// контур заголовка / мета-блока
  if(display.mode() == PLAYER) {
    
    dsp.drawFastHLine(0, 164, 480, config.theme.div); // контур мета-блока / плейлиста

    timekeeper.PrintWeate();
...
void VuWidget::_draw(){
  ...
  uint16_t peakColor = 0xFFE0;//0xFFE0; // YELLOW
...
void ClockWidget::init(WidgetConfig wconf, uint16_t fgcolor, uint16_t bgcolor){
  ...
  if(_fullclock){
    ...
    _clockheight = _timeheight ;//date height
  } else {
...
//*******************    Вывод календарю и иконок погоды   ************************
   ...
   dsp.setFont(&BahnschriftSemiCondensed13);
    ...
    //dsp.setTextSize(2); 
    //snprintf(datebuf, sizeof(datebuf), "%s", utf8Rus(LANG::dow[network.timeinfo.tm_wday], false));
   ...
   // *****************************************************************
      ...
      if(fullClockOnScreensaver){
        //gfx.fillRoundRect(_linesleft, _top()-_timeheight, _timeheight, _timeheight-15, 8, config.theme.seconds);//left line
        //gfx.drawFastHLine(_linesleft-0, _top()-(_timeheight)/2+18+36, CHARWIDTH * _superfont * 2 + _space+7, config.theme.div);//seconds line
  ...
  if(_fullclock || _superfont>0){
   // gfx.setFont(&AvignonProMedium42);
    ...
    if(!_fullclock){
      ...
      gfx.setCursor(_linesleft+_space-4, _top());
...
if(config.store.vumeter) {
    dsp.drawRect(VU_RECT_X_LEFT, VU_RECT_Y, VU_RECT_WIDTH, VU_RECT_HEIGHT, 0xffff);
    dsp.drawRect(VU_RECT_X_RIGHT, VU_RECT_Y+17, VU_RECT_WIDTH, VU_RECT_HEIGHT, 0xffff);
    
     dsp.setFont();
    dsp.setTextSize(1);
    
    dsp .setTextColor(0xFFE0, _bgcolor);
    
    dsp.setCursor(VU_RECT_X_LEFT + 11, VU_RECT_Y + 3);
    dsp.print("L");
    
    dsp.setCursor(VU_RECT_X_RIGHT + 11, VU_RECT_Y + 6+14);
    dsp.print("R");
 ...
 dsp.setFont(&BahnschriftSemiCondensed16);
  ...
  if(_bitrate < 999)  snprintf(_buf, 6, "%dk", _bitrate);
    ...
    snprintf(_buf, 6, "%.1fM", _br);
  }

   dsp.setCursor(BR_LEFT + _dimension/2 - _charWidth*strlen(_buf)/2 -56, BR_TOP +20);
  //dsp.setCursor(BR_LEFT + _dimension/2 - _charWidth*strlen(_buf)/2 + 4, BR_TOP -18);
  dsp.print(_buf);
  //dsp.print(" kB");
  dsp.drawRect(BR_LEFT-60, BR_TOP-1, 125, 30, 0xffff);
  dsp.drawFastVLine (BR_LEFT-1, BR_TOP-1, 29, 0xffff);
  
  switch(_format){
  case BF_MP3: dsp.drawRGBBitmap(BR_LEFT, BR_TOP, mp3, 64, 28);  break;
    case BF_AAC: dsp.drawRGBBitmap(BR_LEFT, BR_TOP, aac, 64, 28);  break;
    case BF_FLAC: dsp.drawRGBBitmap(BR_LEFT, BR_TOP, flac, 64, 28);  break;
    case BF_OGG: dsp.drawRGBBitmap(BR_LEFT, BR_TOP, ogg, 64, 28);  break;
    //case BF_WAV: dsp.drawRGBBitmap(BR_LEFT, BR_TOP, wav, 64, 30);  break;
    case BF_OPUS: dsp.drawRGBBitmap(BR_LEFT, BR_TOP, opus, 64, 28); break;
    case BF_VOR:  dsp.drawRGBBitmap(BR_LEFT, BR_TOP, vor, 64, 28); break;
    default:                        break;
  }
   dsp.setFont();
}
...
void BitrateWidget::_clear() {
  dsp.fillRect(VU_RECT_X_LEFT, VU_RECT_Y, VU_RECT_WIDTH+4, VU_RECT_HEIGHT*2+4,  _bgcolor);
  dsp.fillRect(BR_LEFT-60, BR_TOP-1, 125, 30, _bgcolor);
}
