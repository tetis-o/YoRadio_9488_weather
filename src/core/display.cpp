    ...
    if(pm_result)
      switch (request.type){
        ...
        case NEWWEATHER: {
          ...
          // also redraw the icon/values area right away
          // normally the icons are refreshed once per minute by the clock widget
          // when new weather data arrives we want the screen to show it immediately
          if (config.store.showweather) {
