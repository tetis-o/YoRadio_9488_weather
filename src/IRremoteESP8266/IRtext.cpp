...
IRTEXT_CONST_BLOB_DECL(kAllProtocolNamesStr) {
    D_STR_UNUSED "\x0"
    COND(DECODE_NEC || SEND_NEC, D_STR_NEC, D_STR_UNSUPPORTED) "\x0"
    COND(DECODE_SONY || SEND_SONY, D_STR_SONY, D_STR_UNSUPPORTED) "\x0"
    COND(DECODE_PANASONIC || SEND_PANASONIC, D_STR_PANASONIC, D_STR_UNSUPPORTED) "\x0"
    COND(DECODE_SAMSUNG || SEND_SAMSUNG, D_STR_SAMSUNG, D_STR_UNSUPPORTED) "\x0"
    COND(DECODE_SANYO || SEND_SANYO, D_STR_SANYO, D_STR_UNSUPPORTED) "\x0"
    COND(DECODE_SHARP || SEND_SHARP, D_STR_SHARP, D_STR_UNSUPPORTED) "\x0"
    COND(DECODE_SANYO || SEND_SANYO, D_STR_SANYO_LC7461, D_STR_UNSUPPORTED) "\x0"
    COND(DECODE_NEC || SEND_NEC, D_STR_NEC_LIKE, D_STR_UNSUPPORTED) "\x0"
    COND(SEND_RAW, D_STR_RAW, D_STR_UNSUPPORTED) "\x0"  // SEND-ONLY
    COND(SEND_GLOBALCACHE, D_STR_GLOBALCACHE, D_STR_UNSUPPORTED) "\x0"  // SEND
    COND(DECODE_TOSHIBA_AC || SEND_TOSHIBA_AC, D_STR_TOSHIBA_AC, D_STR_UNSUPPORTED) "\x0"
    COND(DECODE_SAMSUNG_AC || SEND_SAMSUNG_AC, D_STR_SAMSUNG_AC, D_STR_UNSUPPORTED) "\x0"
    COND(DECODE_PANASONIC_AC || SEND_PANASONIC_AC, D_STR_PANASONIC_AC, D_STR_UNSUPPORTED) "\x0"
    COND(DECODE_SAMSUNG36 || SEND_SAMSUNG36, D_STR_SAMSUNG36, D_STR_UNSUPPORTED) "\x0"
    COND(DECODE_TCL112AC || SEND_TCL112AC, D_STR_TCL112AC, D_STR_UNSUPPORTED) "\x0"
    COND(DECODE_SHARP_AC || SEND_SHARP_AC, D_STR_SHARP_AC, D_STR_UNSUPPORTED) "\x0"
    COND(SEND_SONY, D_STR_SONY_38K, D_STR_UNSUPPORTED) "\x0"
    COND(DECODE_SANYO_AC || SEND_SANYO_AC, D_STR_SANYO_AC, D_STR_UNSUPPORTED) "\x0"
    COND(DECODE_PANASONIC_AC32 || SEND_PANASONIC_AC32, D_STR_PANASONIC_AC32, D_STR_UNSUPPORTED) "\x0"
    COND(DECODE_SANYO_AC88 || SEND_SANYO_AC88, D_STR_SANYO_AC88, D_STR_UNSUPPORTED) "\x0"
    COND(DECODE_TCL96AC || SEND_TCL96AC, D_STR_TCL96AC, D_STR_UNSUPPORTED) "\x0"
    COND(DECODE_SANYO_AC152 || SEND_SANYO_AC152, D_STR_SANYO_AC152, D_STR_UNSUPPORTED) "\x0"
